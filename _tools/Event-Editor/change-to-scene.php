...
$eventFile = file_put_contents('../../data/json/story/events/'.$scene.'/'.$event.'.json', file_get_contents("../../data/json/story/events/".$from."/".$event.'.json'), true);
//Delete the file from the original location
unlink('../../data/json/story/events/'.$from.'/'.$event.".json");

//Remove the event from the order in the old scene
$sceneData = json_decode(file_get_contents("../../data/json/story/scenes/".$from.'.json'), true);
if (($key = array_search($event, $sceneData['eventOrder'])) !== false) {
    unset($sceneData['eventOrder'][$key]);
}
file_put_contents("../../data/json/story/scenes/".$from.".json", json_encode($sceneData));

//Add the event to the new scene's event order
$sceneData2 = json_decode(file_get_contents("../../data/json/story/scenes/".$scene.'.json'), true);
$sceneData2['eventOrder'][] = $event;
file_put_contents("../../data/json/story/scenes/".$scene.'.json', json_encode($sceneData2));
